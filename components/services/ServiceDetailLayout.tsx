...
export function ServiceDetailSection({ children, className = "" }: SectionProps) {
  return (
...
export function ServiceDetailFeatureBlock({ title, children, accent }: FeatureBlockProps) {
  return (
    <div className={`relative overflow-hidden rounded-3xl p-10 shadow-sm transition hover:shadow-gold-lg ${
      accent
      ...
      <h2 className={`text-2xl font-bold sm:text-3xl ${accent ? "text-white" : "text-navy"}`}>
        {title}
      </h2>
      <div className={`mt-5 space-y-4 text-base leading-relaxed sm:text-lg ${accent ? "text-grey-200" : "text-grey-600"}`}>
        {children}
      </div>
    </div>
  );
...
export function ServiceDetailBullets({
  ...
  return (
    <div className={`grid gap-3 ${columns === 2 ? "sm:grid-cols-2" : ""}`}>
      {items.map((item) => (
        <div key={item} className="flex items-start gap-3 text-base font-medium text-text-dark sm:text-lg">
...
export function ServiceDetailPricing({ price, description, href = "/contact" }: PricingBlockProps) {
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gold/5 to-gold/[0.02] p-10 ring-1 ring-gold/20 shadow-sm">
      ...
      <div className="relative">
        ...
        <p className="mt-2 text-3xl font-bold text-gold sm:text-4xl">
          {price}
        </p>
        <Link
          href={href}
...
export function ServiceDetailConnector() {
  return (
    <div className="relative h-24 overflow-hidden">
      <svg className="absolute inset-0 w-full h-full text-gold/[0.04]" preserveAspectRatio="none" viewBox="0 0 1440 100" fill="currentColor">
